...
# terminal n'est pas un outil d'analyse. Le journal est écrit au fil de
# l'eau sur un descripteur gardé ouvert, vous pouvez donc le suivre depuis
# un autre terminal pendant que le script tourne :
#
...
usage() {
    cat >&2 <<EOF
usage: ${0##*/} [-d] [-n] [-i] [-m] [-S] [-v] [-c] [-e "motifs"]
  ...
  -S, --strict          vérification stricte : rapatrie et compare octet à
                        ...
                        facturé par Tarsnap au volume transféré.
  -v, --verbose         détaille la progression sur la sortie d'erreur
  -c, --color           colorise le journal. Par défaut il est en texte nu,
                        pour rester grepable ; relisez-le avec "less -R".
...
while getopts "dnimSvcXe:l:f:h" option; do
    case "$option" in
...
for bin in "$TARSNAP_BIN" "$FIND_BIN" "$SORT_BIN" "$JOIN_BIN" "$CMP_BIN" \
           "$SED_BIN" "$GREP_BIN" "$DATE_BIN" "$MKTEMP_BIN"; do
...
log "# date        : $("$DATE_BIN" '+%Y-%m-%d %H:%M:%S')"
log "# repertoires : $BACKUP_DIRS"
log "# exclusions  : $BACKUP_EXCLUDE"
if [ "$NEED_BYTE_CHECK" -eq 0 ]; then
    log "# mode        : metadonnees seules, ni -i ni -m n ayant ete demandes"
    log "#               aucun rapatriement, donc les compteurs = et * sont"
    log "#               indicatifs : ils n ont pas ete verifies octet a octet"
elif [ "$STRICT" -eq 1 ]; then
    log "# mode        : strict, comparaison octet a octet de tous les communs"
else
    log "# mode        : hybride, octet a octet si taille egale et mtime different"
fi
log "#"
log "# <  supprime    >  nouveau    =  identique    *  modifie"
log "#"
...
while IFS="$TAB" read -r path _type _size _stamp; do
    ...
    [ "$SHOW_DELETED" -eq 1 ] && log "${C_RED}< ${path}${C_RESET}"
done < "$DELETED_LIST"

while IFS="$TAB" read -r path _type _size _stamp; do
    [ -n "$path" ] || continue
    new_count=$((new_count + 1))
    [ "$SHOW_NEW" -eq 1 ] && log "${C_GREEN}> ${path}${C_RESET}"
done < "$NEW_LIST"
...
say "Classement des fichiers communs."
{
    while IFS="$TAB" read -r path type_a size_a stamp_a _type_l size_l stamp_l; do
        ...
        if [ "$type_a" != "-" ]; then
            id_count=$((id_count + 1))
            [ "$SHOW_IDENTICAL" -eq 1 ] && log "${C_YELLOW}= ${path}${C_RESET}"
            continue
        fi
        ...
        if [ "$STRICT" -eq 0 ] || [ "$NEED_BYTE_CHECK" -eq 0 ]; then
            if [ "$size_a" != "$size_l" ]; then
                ...
                [ "$SHOW_MODIFIED" -eq 1 ] && log "${C_CYAN}* ${path}${C_RESET}"
                continue
            fi
...
log "#"
log "# supprimes  (<) : $del_count"
log "# nouveaux   (>) : $new_count"
log "# identiques (=) : $id_count"
log "# modifies   (*) : $mod_count"
exec 3>&-
